...
function createRegisteredToolDefinition(client: SlackMCPClient, tool: MCPTool, pi: ExtensionAPI) {
  ...
  return {
    ...
    async execute(
      ...
      try {
        const rawResult = await client.callToolRaw(tool.name, params as Record<string, unknown>);
...
export default function slackMcpExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
      ...
      const config = getEffectiveConfig();
      if (!isNonEmptyString(config.clientId)) {
        return toolError("slack_mcp_connect", buildMissingClientIdError(), {
          connected: false,
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (ctx) applyStatus(ctx, mcpClient, getEffectiveConfig());
        return toolError("slack_mcp_connect", `Connection failed: ${message}`, {
          error: message,
          requiresInteractiveSetup: message === buildMissingClientIdError(),
        });
      }
    },
  });
