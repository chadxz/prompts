...
To attach screenshots or videos to a PR description, use browser use or computer
use to upload them through GitHub's PR description editor. Keep the attachment
links or embed markup generated by GitHub in the final body, including when
updating it later with `gh pr edit`.

Do not check these artifacts into Git. Keep PR screenshots and videos outside
the repository working tree, and never stage or commit them to obtain a URL. If
browser or computer use is unavailable, report the upload blocker and the
missing attachments explicitly.
...
- PR screenshots and videos are uploaded through browser or computer use,
  accessible from the live description, and absent from the Git commit.
