...
const apiKeyRaw = process.env.BRAVE_API_KEY;
if (!apiKeyRaw) {
...
if (apiKeyRaw.startsWith("op://")) {
	try {
		apiKey = execSync(`op read "${apiKeyRaw}"`, { encoding: "utf8" }).trim();
	} catch (e) {
		console.error(`Error: Failed to resolve 1Password reference: ${e.message}`);
		process.exit(1);
	}
}
