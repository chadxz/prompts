...
Use Codex Computer Use with Chrome to upload screenshots and videos through
GitHub's PR description editor. Use `gh` for PR metadata, creation, and text
edits; attachment uploads require the browser UI. The preference for `gh` when
working with GitHub doesn't exclude this upload step. Don't treat `gh` lacking
attachment uploads as a blocker or a reason to omit demo media.

1. Keep the captured media outside the repository working tree. Create the draft
   PR with `gh` first if it doesn't exist yet.
2. Open the PR in Chrome through Codex Computer Use, edit its description, and
   use the editor's attachment control to select the local screenshots and
   videos. Follow the available computer-use tool's instructions for file
   selection.
3. Wait for each upload to finish. Keep GitHub's generated attachment links or
   embed markup in the `Demo` section, add short captions, and save the body.
4. Read the saved body with `gh pr view <url> --json body,url` and check the
   rendered description in Chrome to confirm the images display and videos play.
   Preserve that markup in later `gh pr edit` updates; don't overwrite it with
   the earlier body that lacked attachments.

Do not check these artifacts into Git. Keep PR screenshots and videos outside
the repository working tree, and never stage or commit them to obtain a URL.
Report an upload blocker only when computer use is unavailable or the browser
workflow encounters a concrete failure, such as missing authentication or an
upload error. State what failed and which attachments are missing. A `gh`
limitation alone isn't an upload attempt.
...
- PR screenshots and videos are uploaded through Codex Computer Use with Chrome,
  accessible from the live description, and absent from the Git commit. A `gh`
  attachment limitation hasn't been used to skip the upload workflow.
